...
int main(int argc, char *argv[]) {
  ...
  Statistics *current_stats = new Statistics(
      my_initial_conditions.getNumCases(), my_initial_conditions.getNumDeaths(),
      my_initial_conditions.getPopulation() -
          my_initial_conditions.getNumCases() -
  ...
  QTimer::connect(&timer, &QTimer::timeout, [&]() {
    ...
    for (auto &p : population) {
      ...
      scene->addRect(position->getCorX() - (PERSON_SIZE / 2),
                     position->getCorY() - (PERSON_SIZE / 2), PERSON_SIZE,
                     PERSON_SIZE, {color}, {color});
    }
    scene->update();
