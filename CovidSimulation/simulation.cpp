...
void Simulation::checkTheBoundaries(Person p) {
  if (p.getLocation()->getCorX() - (PERSON_SIZE / 2) < 0 &&
      p.getLocation()->getCorVX() < 0) {
    p.getLocation()->toggleVX();
    p.getLocation()->randomizeVelocity();
  } else if (p.getLocation()->getCorX() + PERSON_SIZE >= SCENE_WIDTH &&
             p.getLocation()->getCorVX() > 0) {
    p.getLocation()->toggleVX();
    p.getLocation()->randomizeVelocity();
  }
  if (p.getLocation()->getCorY() - (PERSON_SIZE / 2) < 0 &&
      p.getLocation()->getCorVY() < 0) {
    p.getLocation()->toggleVY();
    p.getLocation()->randomizeVelocity();
  }

  else if (p.getLocation()->getCorY() + PERSON_SIZE >= SCENE_HEIGHT &&
           p.getLocation()->getCorVY() > 0) {
    p.getLocation()->toggleVY();
    p.getLocation()->randomizeVelocity();
  }
