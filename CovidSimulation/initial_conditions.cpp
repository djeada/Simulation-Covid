...
Initial_conditions::Initial_conditions(QString selected_country) {
  ...
  QString str =
      "C:/Users/patri/OneDrive/Dokumente/CovidSimulation/data/population.2020";
  setPopulation(str);
...
void Initial_conditions::setPopulation(QString str) {
  ...
  while (!s1.atEnd()) {
    ...
    if (contains(s, country) && contains(s, "Total")) {
      ...
      lpopulation.append(
          substring(s, results[results.size() - 3] + 2,
                    results[results.size() - 2] -
                        2)); // appends first column to list, ',' is separator
    }
  }
  file.close();
...
void Initial_conditions::setCasesAndDeaths(QString str) {
  ...
  while (!s1.atEnd()) {
    ...
    if (contains(s, country)) {
      char delimeter = ',';
      QVector<int> results = findLocation(s, delimeter);
      deaths.append(substring(
          s, results[4] + 1,
          results[5] - 1)); // appends first column to list, ',' is separator
...
int Initial_conditions::getNumCases() { return num_cases; }

int Initial_conditions::getNumDeaths() { return num_deaths; }

int Initial_conditions::getPopulation() { return population; }
...
QVector<Person> populate(Initial_conditions my_initial_conditions) {
  ...
  int num_of_infected = (int)my_initial_conditions.getNumCases() * NUM_PEOPLE /
                        my_initial_conditions.getPopulation();
  int num_of_dead = (int)my_initial_conditions.getNumDeaths() * NUM_PEOPLE /
                    my_initial_conditions.getPopulation();
